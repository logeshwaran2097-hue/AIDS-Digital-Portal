...
export function PWAInstall() {
  ...
  return (
    <>
      {/* ── Official Mobile App Installation Prompt ── */}
      {showMobileApkPrompt && !isInstalled && (
        <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white text-slate-900 rounded-3xl p-5 shadow-2xl border border-slate-200 space-y-3.5 animate-in slide-in-from-bottom-5 duration-300 max-h-[90vh] overflow-y-auto">
            {/* Header */}
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl overflow-hidden shadow-[0_0_15px_rgba(0,245,255,0.45)] shrink-0 bg-[#071A41]">
                  <Image
                    src="/icon-192.png"
            ...
            {/* Crucial 'App not installed' Troubleshooting Box */}
            <div className="p-3 rounded-2xl bg-rose-50/80 border border-rose-200/80 text-rose-900 space-y-1.5 text-xs">
              <button
                type="button"
                onClick={() => setShowApkTroubleshoot(!showApkTroubleshoot)}
                className="w-full flex items-center justify-between font-bold text-rose-800 cursor-pointer text-left"
              >
                <span className="flex items-center gap-1.5">
                  <AlertTriangle className="w-3.5 h-3.5 text-rose-600 shrink-0" />
                  Why does phone say "App not installed"?
                </span>
                <span className="text-xs text-rose-600 font-extrabold">{showApkTroubleshoot ? '▲' : '▼'}</span>
              </button>
              
              {showApkTroubleshoot && (
                <div className="space-y-1.5 pt-1 text-[11px] text-rose-800/90 leading-relaxed border-t border-rose-200">
                  <p>
                    <strong>1. Older version already on phone:</strong> Android blocks updating apps if previous versions have different signatures. Go to <strong>Settings → Apps → Digital Portal → Uninstall</strong>, then re-open this APK!
                  </p>
                  <p>
                    <strong>2. Play Protect:</strong> If warned, tap <em>"More details"</em> → <em>"Install anyway"</em>.
                  </p>
                  <p>
                    <strong>3. Best way:</strong> Use <strong>"1-Click Instant Install"</strong> above to install directly without any APK errors.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-center pt-0.5">
              <button
                type="button"
                onClick={handleDismissMobileApkPrompt}
                className="text-xs text-slate-400 hover:text-slate-700 font-semibold transition-colors py-1 cursor-pointer"
              >
                Continue in Browser
              </button>
            </div>
