...
export function GovernmentAttendanceSystem({
  ...
  // Attendance data - initialized with real database students
  ...
  const loadStudents = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.students) {
        ...
        // Once attendance is submitted & locked, it strictly remains locked unless HOD approved unlock
        const shouldLock = !unlockApproved && Boolean(data.existingSession?.isLocked)
  ...
  const handleSave = async (explicitLock?: boolean) => {
    ...
    // Submitting locks the portal session; saving draft leaves it unlocked
    const shouldLock = explicitLock !== undefined ? explicitLock : true
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  {unlockRequest?.status === 'PENDING' ? (
                    ...
                    <button
                      type="button"
                      onClick={() => setShowUnlockModal(true)}
                      className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-xs font-bold border bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white flex items-center justify-center gap-2 transition-all shadow-md shadow-amber-600/20 active:scale-95 cursor-pointer"
                    >
      ...
      <AdvisorODReviewModal
        ...
        onStatusUpdated={(notifId, status) => {
          if (selectedODModal) {
            setStudents((prev) =>
              prev.map((s) => {
                if (s.appliedOD?.id === selectedODModal.id || s.registerNumber === selectedODModal.registerNumber) {
                  return {
                    ...s,
                    status: status === 'endorsed' ? 'OD' : s.status,
                    remarks:
                      status === 'endorsed'
                        ? `OD Endorsed: ${selectedODModal.eventName || selectedODModal.applicationType}`
                        : `OD Declined: ${selectedODModal.eventName || selectedODModal.applicationType}`,
                    appliedOD: {
                      ...s.appliedOD,
                      status: status === 'endorsed' ? 'endorsed_by_advisor' : 'rejected_by_advisor',
                    },
                  }
                }
                return s
