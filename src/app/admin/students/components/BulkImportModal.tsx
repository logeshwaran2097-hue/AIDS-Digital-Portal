...
const AVAILABLE_SECTIONS = ['A', 'B', 'C', 'D']
          ...
          <div className="p-4 sm:p-5 rounded-3xl bg-white border border-blue-100 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              <div className="sm:col-span-2 lg:col-span-4">
                <label className="block text-[11px] font-black text-gray-700 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                  <Building2 className="w-3.5 h-3.5 text-[#003399]" /> Academic Department:
                </label>
                <select
                  value={selectedDepartment}
                  onChange={(e) => setSelectedDepartment(e.target.value)}
                  className="w-full p-2.5 bg-gray-50 hover:bg-white focus:bg-white rounded-xl border border-gray-300 text-xs font-bold text-[#1F2937] focus:border-[#003399] focus:ring-1 focus:ring-[#003399] outline-none transition-all cursor-pointer"
                >
                  ...
                  <tbody className="divide-y divide-gray-100 font-sans">
                    {parsedStudents.slice(0, 100).map((st, idx) => (
                      <tr key={idx} className={st.isValid ? 'hover:bg-gray-50' : 'bg-red-50/50'}>
                        ...
                        <td className="p-2.5 font-mono font-bold text-[#003399]">
                        ...
                        <td className="p-2.5 font-bold text-[#1F2937]">Sec {st.section}</td>
                        <td className="p-2.5">
                          {st.isValid ? (
