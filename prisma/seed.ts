...
async function main() {
  ...
  // Academic year
  const acYear = await prisma.academicYear.upsert({
  ...
  // Seed Years and Semesters
  const yearNames = ['Year 1', 'Year 2', 'Year 3', 'Year 4']
  const yearMap = new Map<number, string>()
  for (let y = 1; y <= 4; y++) {
    const yr = await prisma.year.upsert({
      where: { name_academicYearId: { name: yearNames[y - 1], academicYearId: acYear.id } },
      update: {},
      create: { name: yearNames[y - 1], academicYearId: acYear.id },
    })
    yearMap.set(y, yr.id)
  }
  ...
  for (let sem = 1; sem <= 8; sem++) {
    ...
    const yearId = yearMap.get(yearNum)!
    const semRecord = await prisma.semester.upsert({
      where: { number_yearId: { number: sem, yearId } },
      update: { name: `Semester ${sem}` },
      create: { name: `Semester ${sem}`, number: sem, yearId },
    })
    semesterMap.set(sem, semRecord.id)
  }

  // Official AI&DS Curricular Subjects
  ...
  for (const c of curriculumCourses) {
    const semId = semesterMap.get(c.sem)
    const yrId = yearMap.get(Math.ceil(c.sem / 2))
    const meta = JSON.stringify({
      semester: c.sem,
      category: c.category,
      facultyInCharge: c.faculty,
      courseType: c.courseType,
      exactCredits: c.credits,
      notes: `${c.code} - ${c.name}`,
    })
  ...
  console.log(`Successfully seeded ${curriculumCourses.length} official curriculum subjects!`)
